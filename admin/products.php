...
if (isset($_GET['delete'])) {
  ...
  $row->execute([$pid]);
  $row = $row->fetch();
  if ($row && $row['image']) @unlink(UPLOAD_DIR . $row['image']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$error) {
    if ($id) {
      $db->prepare("UPDATE products SET product_name=?,slug=?,description=?,price=?,stock_quantity=?,category_id=?,image=?,is_featured=? WHERE id=?")
        ->execute([$name, $slug, $description, $price, $stock, $catId, $imageName, $featured, $id]);
    } else {
      $db->prepare("INSERT INTO products (product_name,slug,description,price,stock_quantity,category_id,image,is_featured) VALUES (?,?,?,?,?,?,?,?)")
        ->execute([$name, $slug, $description, $price, $stock, $catId, $imageName, $featured]);
    }
...
$products   = $db->query("SELECT p.*, c.category_name FROM products p LEFT JOIN categories c ON c.id=p.category_id ORDER BY p.created_at DESC")->fetchAll();
      ...
      <div class="col-lg-4">
        <div class="card border-0 shadow-sm rounded-4 p-4">
          ...
          <form method="POST" enctype="multipart/form-data">
              ...
              <select name="category_id" class="form-select">
                ...
                <?php foreach ($categories as $c): ?>
                  <option value="<?= $c['id'] ?>" <?= ($editing['category_id'] ?? '') == $c['id'] ? 'selected' : '' ?>>
                    <?= htmlspecialchars($c['category_name']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="mb-3">
              <label class="form-label fw-semibold">Product Image</label>
              <?php if (!empty($editing['image']) && file_exists(UPLOAD_DIR . $editing['image'])): ?>
                <img src="<?= UPLOAD_URL . htmlspecialchars($editing['image']) ?>" id="img-preview"
                  style="width:100%;border-radius:10px;margin-bottom:.5rem" />
              <?php else: ?>
                <img id="img-preview" style="display:none;width:100%;border-radius:10px;margin-bottom:.5rem" />
              <?php endif; ?>
            ...
            <div class="d-flex gap-2">
              <button class="btn btn-primary-custom flex-grow-1"><?= $editing ? 'Update Product' : 'Add Product' ?></button>
              <?php if ($editing): ?>
                    ...
                    <tr>
                      <td>
                        <?php if ($p['image'] && file_exists(UPLOAD_DIR . $p['image'])): ?>
                          <img src="<?= UPLOAD_URL . htmlspecialchars($p['image']) ?>" style="width:50px;height:50px;object-fit:cover;border-radius:8px" />
                        <?php else: ?>
                      ...
                      <td><?= $p['is_featured'] ? '⭐' : '—' ?></td>
                      <td>
                        <a href="products.php?edit=<?= $p['id'] ?>" class="btn btn-sm btn-outline-primary me-1">Edit</a>
                        <a href="products.php?delete=<?= $p['id'] ?>" class="btn btn-sm btn-outline-danger confirm-delete">Delete</a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
