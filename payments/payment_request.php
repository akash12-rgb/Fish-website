...
if ($buyNowId) {
    ...
    $product = $st->fetch();
    if (!$product) { die('Product unavailable.'); }
    $orderItems = [[
...
try {
    ...
    ');
    $st->execute([
        $user['id'], $total, 'pending', 'pending', $transactionId,
        $shippingName, $shippingPhone, $shippingAddress,
        $shippingCity, $shippingZip, $notes,
    ]);
    $orderId = $st->fetchColumn();

    // 2. Insert order items & decrement stock
    $itemSt  = $db->prepare('INSERT INTO order_items (order_id,product_id,quantity,price) VALUES (?,?,?,?)');
    $stockSt = $db->prepare('UPDATE products SET stock_quantity = stock_quantity - ? WHERE id=?');
    foreach ($orderItems as $item) {
        $itemSt->execute([$orderId, $item['product_id'], $item['quantity'], $item['price']]);
        $stockSt->execute([$item['quantity'], $item['product_id']]);
    }

    // 3. Create payment record
    $st = $db->prepare('INSERT INTO payments (order_id,payment_gateway,transaction_id,amount,payment_status) VALUES (?,?,?,?,?)');
    $st->execute([$orderId, 'ICICI_OrangePay', $transactionId, $total, 'pending']);
...
// Checksum = SHA256 of: merchant_id|order_id|amount|currency|merchant_key
...
// ── Build payload and POST/redirect ──────────────────────────
$params = [
...
// In production you would POST directly (server-side cURL) and redirect to the URL
// returned. Here we use an auto-submitting HTML form which is the most common pattern:
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    body { font-family: sans-serif; display: flex; flex-direction: column;
           ...
           background: #f0f9f7; color: #0a1628; }
    .spinner { width: 60px; height: 60px; border: 5px solid #e2e8f0;
               border-top-color: #1a7a6e; border-radius: 50%;
...
<body>
  ...
  <p style="color:#718096">Please do not close or refresh this page.</p>
