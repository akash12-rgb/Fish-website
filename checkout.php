    ...
    <div class="row g-4">
      ...
      <div class="col-lg-7">
        <div class="checkout-step">
          ...
          <div class="row g-3">
            ...
            <div class="col-12">
              ...
              <textarea name="notes" class="form-control" rows="2" placeholder="Delivery instructions, etc."></textarea>
            </div>
          </div>
        </div>
        ...
        <div class="checkout-step">
          ...
          <div class="p-3 rounded" style="border:2px solid var(--primary);background:var(--light-bg)">
            <div class="d-flex align-items-center gap-3">
              <span style="font-size:2rem">🏦</span>
              <div>
                <div class="fw-bold">ICICI Bank Orange Pay</div>
                <small class="text-muted">Secure payment via ICICI Orange Pay Gateway</small>
              </div>
              <i class="bi bi-check-circle-fill ms-auto" style="color:var(--primary);font-size:1.4rem"></i>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="cart-summary-box" style="position:sticky;top:90px">
          ...
          <?php foreach ($items as $item): ?>
            <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
              <div>
                <div class="small fw-semibold"><?= htmlspecialchars($item['product_name']) ?></div>
                <small class="text-muted">× <?= $item['quantity'] ?></small>
              </div>
              <div class="small fw-bold">Rs <?= number_format($item['price'] * $item['quantity'], 0, ',', '.') ?></div>
            </div>
          <?php endforeach; ?>
          <div class="order-line mt-2"><span>Subtotal</span><span>Rs <?= number_format($subtotal, 0, ',', '.') ?></span></div>
          ...
          <div class="order-total">
            <span>Total</span>
            <span style="color:var(--primary)">Rs <?= number_format($total, 0, ',', '.') ?></span>
          </div>
          <button type="submit" class="btn btn-primary-custom w-100 mt-3 py-3" style="font-size:1.05rem">
            🔒 Proceed to Payment
          </button>
          <p class="text-center text-muted small mt-2">You will be redirected to ICICI Orange Pay</p>
        </div>
      </div>
    </div>
  </form>
